...
public class GoogleLoginStrategy implements OAuth2LoginStrategy {
    ...
    public Authentication doAuthenticate(SignInRequest requestLogin) {
        ResponseSocialUserInfo googleUser = userSocialLinkService.getSocialUserInfo(AuthProviderEnum.GOOGLE, requestLogin.getAccessToken());
        User user = userSocialLinkService.handleUserBySocialAccount(AuthProviderEnum.GOOGLE, googleUser.getSub(), googleUser.getEmail(), requestLogin.getAccessToken());
